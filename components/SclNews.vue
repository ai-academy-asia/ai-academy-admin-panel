    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
      ...
      >
        <div class="flex flex-row justify-between px-4 items-center">
          <div class="flex gap-2 items-center">
            <div class="size-8 rounded-full bg-coreBgMain text-sm text-coreSecondaryText flex items-center justify-center">
              <img v-if="item.created_by?.image" class="size-8 rounded-full object-cover" :src="`/api/file/${(item.created_by?.image?._id || item.created_by?.image)}`">
        ...
        <div class="px-4">
          ...
          <img class="w-full aspect-video object-cover" :src="`/api/file/${(item.image?._id || item.image)}`">
          <!-- </nuxt-link> -->
        </div>

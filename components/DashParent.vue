<template>
  <div class="w-full flex flex-col items-center pb-40">
    <div class="flex flex-col gap-4 max-w-2xl w-full">
      <div class="p-2 sm:p-4 rounded bg-white items-center flex flex-row gap-2 flex-wrap">
        <div class="flex-1 min-w-52">
          Та <b>{{ parent.student_id?.given_name }}</b>-н <b>{{ parent.who_id.name }}</b> эрхтэй байна.
        </div>
        <div class="flex flex-row gap-2">
          <div>
            <el-button size="small" @click="studentAttendance = parent.student_id">
              <div class="flex flex-row items-center gap-1">
                <i class="isax isax-timer-1 text-base" />
                Ирц үзэх
              </div>
            </el-button>
          </div>
          <div>
            <el-button size="small" @click="studentScore = parent.student_id">
              <div class="flex flex-row items-center gap-1">
                <i class="isax isax-discount-shape text-base" />
                Дүн үзэх
              </div>
            </el-button>
          </div>
      ...
      <div v-for="item in list" :key="item._id" class="rounded bg-white shadow-lg w-full gap-2 flex flex-col py-4">
        ...
        <div v-if="item.description?.toLowerCase().includes('#дүн') || item.description?.toLowerCase().includes('#ирц')" class="px-4">
          <el-button v-if="item.description?.toLowerCase().includes('#ирц')" plain size="mini" @click="studentAttendance = parent.student_id">
            Ирц үзэх
          </el-button>
          <el-button v-if="item.description?.toLowerCase().includes('#дүн')" plain size="mini" @click="studentScore = parent.student_id">
            Дүн үзэх
          </el-button>
        </div>
    ...
    <dash-parent-attendance v-model="studentAttendance" />
    <dash-parent-score v-model="studentScore" />
  </div>
</template>
<script>
...
export default {
  name: 'DashTeacher',
  props: {
    parent: { type: Object, default: () => { return null } }
  },
  data () {
    return {
      listAll: [],
  ...
  computed: {
    ...
    list () {
      const list = [...this.listAll, ...this.listStudents]
  ...
  methods: {
    async handleRefresh () {
      try {
        ...
        this.listStudents = []
        this.isMore = false
        const listAll = await this.getInformationAll()
        const listStudents = await this.getInformationStudents()
        this.isMore = listAll.length >= 10 || listStudents.length >= 10
    ...
    async handleMore () {
      try {
        ...
        const listStudents = await this.getInformationStudents()
        this.isMore = listAll.length >= 10 || listStudents.length >= 10
        this.listAll = [...this.listAll, ...listAll]
        this.listStudents = [...this.listStudents, ...listStudents]
      } catch (err) {
        this.$showError(err)
      } finally {
        this.loading = false
    ...
    async getInformationAll () {
      const { data: informations } = await this.$axios.get('list/8/informations', {
        params: {
          ...
          search: {
            is_all: { val: true, t: 'boolean', op: 'eq' }
          }
        }
      })
    ...
    async getInformationStudents () {
      const { data: informations } = await this.$axios.get('list/8/informations', {
        params: {
          pageSize: 10,
          extraFields: 'created_by.image,created_by.full_name',
          search: {
            is_all: { val: false, t: 'boolean', op: 'eq' },
            students: { val: this.parent.student_id._id, t: 'number', op: 'eq' }
          }
        }
      })
      if (informations.length > 0) {
